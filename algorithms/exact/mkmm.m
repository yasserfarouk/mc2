...
function [locs,...
    dists,executionTime,means,stats]=mkmm(x,lrange,lStep,normalization,innerOverlap ...
                        ,nMotifs,outerOverlap,R,recalcDists,approximate,earlyAbandon,motifCopyingStrategy)
...
% motifCopyingStrategy      1   We will use 2*nMotifs in the longest length
%                           2   We will use nMotifLengths*nMotifs in the
%                                   longest length
%
%
%
...
%           the motif
% executionTime The execution time reported by the c implementation
% dists     an m*1 vector giving the distances associated with discovered
%           motifs
...
%           problem with mk++.exe) it returns zero
    ...
    str=sprintf('mk--.exe tmpMKData.csv %d %d %d %d %f %d %d %f %d %d %d %d %d', ...
        length(x),lrange(1),lrange(2),lStep,innerOverlap,R,nMotifs,outerOverlap...
        ,recalcDists,earlyAbandon,normalization,motifCopyingStrategy,approximate);
    [s,result]=system(str);    
    cd(current);
    if s
        executionTime=inf;

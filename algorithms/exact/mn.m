...
function [locs,...
    dists,executionTime,means,stats]=mn(x,lrange,lStep,normalization,innerOverlap ...
...
%           problem with mk++.exe) it returns zero
    ...
    str=sprintf('mn.exe tmpMKData.csv %d %d %d %d %f %d %d %f %d %d %d %d', ...
        length(x),lrange(1),lrange(2),lStep,innerOverlap,R,nMotifs,outerOverlap...
        ,recalcDists,earlyAbandon,normalization,approximate);
    [s,result]=system(str);    
    cd(current);
    if s
        executionTime=inf;

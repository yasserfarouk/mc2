...
load('./Data/stats.mat');
if doFinalizations && showFinalizationResults
...
if showFinalizationResults
    ...
    for a=1:nAlgs
        ...
        mymean=mean(tmp,1);
        mystd=std(tmp,1);    
        h=subplot(1,3,1,'XScale','log');
        hold on;
        plot(noises,mymean,colors{a});
...
utimes=times
if useNormalizedTimes
	for s=1:nSignals
        for n=1:nNoises
            for l=1:nLocNoises
				utimes(s,n,l,:)=utimes(s,n,l,:)./max(utimes(s,n,l,:));
			end
		end
	end
...
for a=1:nAlgs
    ...
    for s=1:nSignals
        for n=1:nNoises
            for l=1:nLocNoises
                allTimes(nxt,a)=utimes(s,n,l,a);
                if showFinalizationResults
                    allTimesF(nxt,a)=utimesF(s,n,l,a);
                end
                nxt=nxt+1;
            end
        end
...
figure
h=subplot(1,1+showFinalizationResults,1);
...
if showFinalizationResults    
    h=subplot(1,+showFinalizationResults,2);
    boxplot(allTimesF,'notch','on');
    set(h,'XTickLabel',algNames,...
        'XTick',[1 2 3 4 5]);
...
legend(algNames);
ylabel('Execution Time in ms/point');
title('Execution Time as a function of noise level');

if showFinalizationResults    
    h=subplot(1,+showFinalizationResults,2);
    hold on;
    for a=1:nAlgs
        tmp=(squeeze((timesF(:,:,1,a))));
        mymean=mean(tmp,1);
        mystd=std(tmp,1);            
        plot(noises,mymean,colors{a},'LineWidth',2);
        %set(gca,'XScale','log','FontSize',24);
    ...
    legend(algNames);
    ylabel('Execution Time in ms/point');
    title('Finalization Time as a function of noise level');
end
...
legend(algNames);
ylabel('Normalized Execution Time as a fraction of the slowest algorithm');
title('Normalized Execution Time as a function of noise level');

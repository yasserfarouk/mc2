function h=grPlot(V,E,kind,vkind,ekind,as)
...
%   as arrow size (used only with digraphs). can be any positive real
%   number
% Output parameter:
  ...
  while k<m2,
    ...
    for k2=1:k1, % we draw the edges (arrows)
      ...
      if kind1=='d', % digraph with arrows
        if E2(k+k2-1,1)==1,
          [xa,ya]=CreateArrow(MyXg(1:2),MyYg(1:2),arrowSize);
          fill(xa,ya,'k');
        else
          [xa,ya]=CreateArrow(MyXg(end:-1:end-1),MyYg(end:-1:end-1),arrowSize);
          fill(xa,ya,'k');
        end
      end
...
%        set(hhh,'FontName','Times New Roman Cyr','FontSize',18)
  ...
  while k<ml,
    ...
    for k2=1:k1, % we draw the loop
      ...
      plot(MyX,MyY,'k-');
      if kind1=='d',
        [xa,ya]=CreateArrow(MyX([1 10]),MyY([1 10]),arrowSize);
        fill(xa,ya,'k');
      end
      if ~isempty(ekind1), % labels of edges (arrows)
        if we==3,
...
function [xa,ya]=CreateArrow(x,y,arrowSize)
% create arrow with length 0.1 with tip x(1), y(1) 
% and direction from x(2), y(2)

xa1=[0 0.1*arrowSize 0.08 0.1*arrowSize 0]';
ya1=[0 0.03*arrowSize 0 -0.03*arrowSize 0]';
dx=diff(x);
dy=diff(y);
alpha=atan2(dy,dx); % angle of rotation
cosa=cos(alpha);
